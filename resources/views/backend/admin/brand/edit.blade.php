        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="d-flex justify-content-between m-0">Edit Brand
                        <a href="{{ route('admin.brand.index') }}" class="btn btn-sm btn-primary waves-effect"><i class="mdi mdi-arrow-left-thick"></i> Back</a>
                    </h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.brand.update', $Edit->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                <label>Brand Name</label>
                                <input type="text" name="name" value="{{ $Edit->name }}" class="form-control @error('name') is-invalid @enderror" placeholder="Brand name" required>
                                @error('name')
